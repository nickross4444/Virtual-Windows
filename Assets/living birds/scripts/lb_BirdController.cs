...
public class lb_BirdController : MonoBehaviour {
    ...
    private void Update()
    {
        if(activeBirds>0)
		{
			...
			{
				timer = 0f;
				if (spawnedBirds.Count == 0) return;
				spawnedBirds[0].SetActive(false);
	...
	void SpawnBird(){
		if (!pause){
            ...
            bird.transform.position = FindMyPoint();
	...
	void BirdFindTarget(GameObject bird){
		...
		GameObject target=PerchTarget.gameObject;
		if(target.transform!=PerchTarget)
		{
            bird.SendMessage("FlyToTarget", PerchTarget.position);
        }
		if (birdGroundTargets.Count > 0 || birdPerchTargets.Count > 0){
